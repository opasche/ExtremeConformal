...
conformalize_EQR_unilat <- function(EQR_pred, y_calibr, coverage_proba=NULL, coverage_alpha=NULL, 
                                    ...
                                    alpha_profile_naive=0.01, return_q_list=FALSE, verbose=1) { 
  # TODO: Other alpha_corrections? 'Sidak','Bonferroni','Holm-Bonferroni','Hochberg','Hommel','Benjamini-Hochberg','Benjamini-Yekutieli','Storey-Tibshirani','FDR','FWER'
  ...
  if (method == 'Empirical') {
    dQ_res_list <- conformalize_Empirical(s_cal=s_cal, coverage_proba=coverage_proba, verbose=verbose)
    
  } else if (method == 'GPD_simple') {
    dQ_res_list <- conformalize_GPD_max(s_cal=s_cal, coverage_proba=coverage_proba, 
                                        threshold_lvls=threshold_lvls, min_obs_GPD=0, verbose=verbose)
    
  } else if (method == 'GPD_max') {
    dQ_res_list <- conformalize_GPD_max(s_cal=s_cal, coverage_proba=coverage_proba, 
                                        threshold_lvls=threshold_lvls, min_obs_GPD=min_obs_GPD, verbose=verbose)
    ...
    dQ_res_list <- conformalize_GPD_profile(s_cal=s_cal, coverage_proba=coverage_proba, threshold_lvls=threshold_lvls, 
                                            ...
                                            steps_beyond_conf=profile_steps_beyond_conf, max_steps=profile_max_steps, verbose=verbose)
...
conformal_PI_unilat <- function(Q_pred, unilat_conformalizer, return_format=c('upper_limit','interval','text'), 
                                coverage_proba=NULL, coverage_alpha=NULL){
  ...
  if(return_format=='upper_limit'){
    ...
    res <- data.frame(PI_down=rep(-Inf,n),
                      PI_up=c(PI_up))
    if(!is.null(coverage_proba)){
    ...
    res <- paste0('Prediction Interval: (-Inf, ', PI_up, ')')
    if(!is.null(coverage_proba)){
...
#' Classical split conformalization procedure
#'
#' @param s_cal .
...
#'
#' @keywords internal
conformalize_Empirical <- function(s_cal, coverage_proba, verbose=1) {
  n <- length(s_cal)
...
#' GPD-max conformalization procedure
#'
#' @param s_cal .
#' @param coverage_proba .
...
#'
#' @keywords internal
conformalize_GPD_max <- function(s_cal, coverage_proba, threshold_lvls=NULL, min_obs_GPD=10, verbose=1){
  n <- length(s_cal)
  if(is.null(threshold_lvls)){
    threshold_lvls <-  seq(0.8,0.99,0.01)
  }
...
#' GPD-boot conformalization procedure
#'
#' @param s_cal .
#' @param threshold_lvls .
#' @param alpha_profile .
...
#'
#' @keywords internal
conformalize_GPD_boot <- function(s_cal, threshold_lvls, alpha_profile, alpha_profile_qlvl, R=1004, verbose=1){
  n <- length(s_cal)
  ...
  # GPD_extRemes <- extRemes::fevd(s_cal, threshold=u, type="GP", method='MLE', time.units="days", period.basis="year")
  # bci100 <- extRemes::ci.fevd(GPD_extRemes, threshold=u, alpha=alpha_profile, type="return.level", 
...
#' GPD-delta conformalization procedure
#'
#' @param s_cal .
#' @param threshold_lvls .
#' @param alpha_profile .
...
#'
#' @keywords internal
conformalize_GPD_delta <- function(s_cal, threshold_lvls, alpha_profile, alpha_profile_qlvl, verbose=1){
  n <- length(s_cal)
...
#' GPD-profile conformalization procedure
#'
#' @param s_cal .
#' @param coverage_proba .
#' @param threshold_lvls .
